...
class Tree(object):
    ...
    def make(self):
        ...
        self.tree_obj = bpy.data.objects.COMPAT_new('Tree', None)
    ...
    def create_branches(self):
        ...
        COMPAT_enumerated = enumerate(['Trunk'] + ['Branches' + str(_) for _ in range(1, self.param.levels)])
        originalStatement = 'for level_depth, level_name in COMPAT_enumerated:'
        for level_name in COMPAT_enumerated:
            level_curve = bpy.data.curves.COMPAT_new(level_name.lower(), type='CURVE')
            ...
            level_obj = bpy.data.objects.COMPAT_new(level_name, level_curve)
        ...
        for ind in range(self.param.branches[0]):
            ...
            originalStatement = "trunk = self.branch_curves[0].splines.COMPAT_new('BEZIER')"
            #trunk = self.branch_curves[0].splines.COMPAT_new('BEZIER')
    ...
    def create_leaf_mesh(self):
        ...
        # set up mesh object
        if leaf_index > 0:
            leaves = bpy.data.meshes.COMPAT_new('leaves')
            leaves_obj = bpy.data.objects.COMPAT_new('Leaves', leaves)
            ...
            if leaf_uv:
                leaves.uv_layers.COMPAT_new(name="leavesUV")
                uv_layer = leaves.uv_layers.active.data

                for seg_ind in range(int(len(leaf_faces) / len(base_leaf_shape[1]))):
                    vert_ind = 0
                    for vert in leaf_uv:
                        originalStatement = 'uv_layer[seg_ind * len(leaf_uv) + vert_ind].uv = vert'
                        #uv_layer[seg_ind * len(leaf_uv) + vert_ind].uv = vert
                        vert_ind += 1
                        # leaves.validate()

        if blossom_index > 0:
            blossom = bpy.data.meshes.COMPAT_new('blossom')
            blossom_obj = bpy.data.objects.COMPAT_new('Blossom', blossom)
        ...
        for seg_ind in range(start, curve_res + 1):
            ...
            if self.param.curve_v[depth] < 0:
                ...
                if seg_ind == 0:
                    ...
                    new_point = stem.curve.bezier_points[COMPAT_MINUS_ONE]
                    ...
                    if seg_ind == 1:
                        ...
                        prev_point = stem.curve.bezier_points[COMPAT_MINUS_TWO]
    ...
    def make_clones(self, turtle, seg_ind, split_corr_angle, num_branches_factor, clone_prob,
        ...
        using_direct_split = self.param.split_angle["COMPAT_stem.depth"] < 0
        stem_depth = self.param.split_angle_v["COMPAT_stem.depth"]
        ...
        for split_index in range(num_of_splits):
            ...
            originalStatement ="split_stem = self.branch_curves[stem.depth].splines.COMPAT_new('BEZIER')"
            ...
            if self.param.split_angle_v["COMPAT_stem.depth"] >= 0:
                cloned = turtle
            else:
                cloned = None
    ...
    def make_branches(self, turtle, stem, seg_ind, branches_on_seg, prev_rotation_angle, is_leaves=False):
        ...
        start_point = stem.curve.bezier_points[COMPAT_MINUS_TWO]
        end_point = stem.curve.bezier_points[COMPAT_MINUS_ONE]
        branches_array = []
        ...
        if branches_on_seg < 0:  # fan branches
            ...
            base_length = stem.length * self.param.base_size["COMPAT_stem.depth"]
            branch_dist = self.param.branch_dist[d_plus_1]
            curve_res = int(self.param.curve_res["COMPAT_stem.depth"])
        ...
        if is_leaves:
            originalStatement = "for pos_tur, dir_tur, _, _ in branches_array:"
            for dir_tur in branches_array:
                self.leaves_array.append(Leaf(pos_tur.pos, dir_tur.dir, dir_tur.right))
        else:
            originalStatement = "for pos_tur, dir_tur, rad, b_offset in branches_array:"
            for dir_tur in branches_array:
                originalStatement = "new_spline = self.branch_curves[d_plus_1].splines.COMPAT_new('BEZIER')"
                #new_spline = self.branch_curves[d_plus_1].splines.COMPAT_new('BEZIER')
    ...
    def set_up_branch(self, turtle, stem, branch_mode, offset, start_point,
        ...
        branch_dir_turtle = make_branch_dir_turtle(turtle, self.param.curve_v["COMPAT_stem.depth"] < 0, offset, start_point,
                                                   end_point)
    ...
    def calc_stem_radius(self, stem):
        ...
        if stem.depth == 0:  # trunk
            ...
            result = self.param.radius_mod["COMPAT_stem.depth"] * stem.parent.radius * pow((
        ...
        if self.param.down_angle_v[d_plus_1] >= 0:
            ...
            d_angle = self.param.down_angle[d_plus_1] + (self.param.down_angle_v[d_plus_1] * (
                1 - 2 * self.shape_ratio(0, (stem.length - stem_offset) / (stem.length * (
                    1 - self.param.base_size["COMPAT_stem.depth"])))))
    ...
    def calc_branch_count(self, stem):
        ...
        return result / (1 - self.param.base_size["COMPAT_stem.depth"])
    ...
    def radius_at_offset(self, stem, z_1):
        ...
        n_taper = self.param.taper["COMPAT_stem.depth"]
    ...
    def increase_bezier_point_res(self, stem, seg_ind, points_per_seg):
        ...
        curve_res = int(self.param.curve_res["COMPAT_stem.depth"])
        seg_end_point = stem.curve.bezier_points[COMPAT_MINUS_ONE]
        ...
        seg_start_point = stem.curve.bezier_points[COMPAT_MINUS_TWO]
        ...
        for k in range(0, points_per_seg):
            ...
            if k == 0:
                ...
                if k == 1:
                    ...
                    stem.curve.bezier_points.add(1)
                    curr_point = stem.curve.bezier_points[COMPAT_MINUS_ONE]
