...
export function calc_point_on_bezier(offset, start_point, end_point) {
    ...
    const rightPartialCoPartial = start_point.co.clone().multiplyScalar(Math.pow(one_minus_offset, 3));
    const rightPartialRightPartial = start_point.handle_right.clone().multiplyScalar(3 * Math.pow(one_minus_offset, 2) * offset);
    const rightPartial = rightPartialCoPartial.add(rightPartialRightPartial);

    const leftPartial = end_point.handle_left.clone().multiplyScalar((3 * one_minus_offset) * Math.pow(offset, 2));
...
export  function calc_tangent_to_bezier(offset, start_point, end_point) {
    ...
    const startPartial =startDiff.multiplyScalar(3 * Math.pow(one_minus_offset, 2));
...
export  function calc_helix_points(turtle, rad, pitch) {
    ...
    points = [new Vector([0, (- rad), ((- pitch) / 4)]), new Vector([((4 * rad) / 3), (- rad), 0]), new Vector([((4 * rad) / 3), rad, 0]), new Vector([0, rad, (pitch / 4)])];
...
export function point_in_cube(point) {
    const size = 2;
    return (((abs(point.x) < size) && (abs(point.y) < size)) && (abs((point.z - size)) < size));
}
...
export function scale_bezier_handles_for_flare(stem, max_points_per_seg) {
    ...
    stem.curve.bezier_points.forEach(point => {
        const leftPartial = point.handle_left.clone().sub(point.co).divideScalar(max_points_per_seg);
        point.handle_left = leftPartial.add(point.co);

        const rightPartial = point.handle_right.clone().sub(point.co).divideScalar(max_points_per_seg);
